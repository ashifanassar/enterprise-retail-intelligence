...
def extract_product_from_query(query: str, session_id: str) -> list[dict]:
    clean = query.lower()
    for word in ["add", "put", "place", "to cart", "to my cart", "remove", "delete", "from cart", "please", "can you"]:
...
def log_cart_event(
    ...
    try:
        bigquery_client.insert_rows_json(f"{PROJECT_ID}.{BQ_DATASET}.cart_events", rows)
    except Exception:
        pass


@traceable(name="Transaction Executor")
def transaction_executor_node(state: GraphState) -> GraphState:
    ...
    try:
        if event_type == "add":
            ...
            if not products:
                response = "I couldn't find that product in our catalogue. Please search for it first."
            else:
                product = products[0]
                ...
                log_cart_event(session_id, "add", cart, risk_score, hitl, product["sku_id"])
            ...
            if cart["items"]:
                ...
                response = f"Removed '{title}' from cart. Cart total: Rs.{cart['total']:.0f}."
            else:
            ...
            code = next((word for word in query.upper().split() if word in promo_codes), None)
            if code:
                cart, discount_pct = apply_promo(session_id, code)
                risk_score, hitl, reasons = calculate_risk_score(cart, "promo", discount_pct)
                if hitl:
                    response = "Your promo request is pending policy review."
                else:
                    response = f"Promo code {code} applied. New cart total: Rs.{cart['total']:.0f}."
                log_cart_event(session_id, "promo", cart, risk_score, hitl, promo=code, discount=discount_pct)
            else:
                response = "Please provide a valid promo code: SAVE10, SAVE20, SAVE30, or WELCOME."

        elif event_type == "checkout":
            if not cart["items"]:
                response = "Your cart is empty. Add products before checking out."
            else:
                risk_score, hitl, reasons = calculate_risk_score(cart, "checkout", cart.get("discount_pct", 0))
                if hitl:
                    response = "Your checkout is pending policy review."
                else:
                    response = f"Order confirmed. Total: Rs.{cart['total']:.0f}. Delivery in 3-5 business days."
                log_cart_event(session_id, "checkout", cart, risk_score, hitl)

        else:
            if cart["items"]:
                items_text = "\n".join(
                    f"- {item['title']} x{item['quantity']} = Rs.{item['price'] * item['quantity']:.0f}"
        ...
        return {**state, "response": response, "error": str(exc), "intent": "transact"}
